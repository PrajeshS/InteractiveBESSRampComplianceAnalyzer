...
event_notes = {
    ...
    56: "<b>Highest Variability:</b> Contains a 97MW production drop. Tests Power (MW) response.",
    331: "<b>Largest SOC Swing:</b> Deepest daily energy cycle. Tests Energy (MWh) capacity.",
    75: "<b>Highest Solar Generation:</b> Peak annual energy (1072.5 MWh). Tests BESS saturation risks.",
    346: "<b>Lowest Solar Generation:</b> Minimum annual energy (74.5 MWh). Verifies behavior during low-output periods."
}
...
def run_sim(pv_data, p_cap, e_cap, s_min, s_max, s_day, start_soc_pct, eff, threshold):
    ...
    t_solar, t_export, t_curtail_inherent, t_curtail_ramp, t_bess_mwh = 0, 0, 0, 0, 0
    ...
    for t in range(n):
        ...
        t_curtail_inherent += max(0, pv - 100.0) / 60
        ...
        # Ramp curtailment is solar under 100MW that couldn't be exported due to ramp limits
        t_curtail_ramp += max(0, raw_pv_capped - exp - (actual_bess if actual_bess > 0 else 0)) / 60
        t_export += exp / 60
    ...
    return grid_export, bess_pwr, soc_history, violations, day_mins, t_solar, t_export, t_curtail_inherent, t_curtail_ramp, t_bess_mwh, d_solar, d_export, d_curtail, d_bess_mwh
...
c8.metric('Ramp Event Curtailment', f'{a_curt_ramp:,.0f} MWh')
...
fig.update_layout(
    ...
    yaxis2=dict(overlaying='y', side='right', range=[0,100], title='SOC %'),
