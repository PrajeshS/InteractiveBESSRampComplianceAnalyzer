...
fig = go.Figure()
fig.add_trace(go.Scattergl(x=times, y=pv_signal[s:e], name='Raw Solar (MW)', line=dict(color='#8b949e', dash='dot')))
fig.add_trace(go.Scattergl(x=times, y=export[s:e], name='Net Export (MW)', line=dict(color='#58a6ff', width=2)))
fig.add_trace(go.Scattergl(x=times, y=bess[s:e], name='BESS (MW)', fill='tozeroy', line=dict(color='#238636', width=1)))
fig.add_trace(go.Scattergl(x=times, y=soc[s:e], name='SOC (%)', yaxis='y2', line=dict(color='#f2cc60', width=2)))
