...
def run_sim(pv_data, p_cap, e_cap, s_min, s_max, s_day, start_soc_pct, eff, threshold):
    ...
    for t in range(n):
        ...
        if t >= d_start and t < d_end:
            d_solar, d_export, d_curtail, d_bess_mwh = d_solar + pv/60, d_export + exp/60, d_curtail + curt/60, d_bess_mwh + abs(actual_bess)/60
...
c4.metric('Annual Full Cycles', f'{a_bess_mwh / (2 * enr_cap * (soc_max-soc_min)):.1f}')
...
fig.update_layout(hovermode='x unified', yaxis2=dict(overlaying='y', side='right', range=[0,100], title='SOC %'), template='plotly_dark', height=550, legend=dict(orientation='h', y=1.1))
